...
png(file="PDP(RF and boosting).png", height=1800, width=1800, units = "px", res=200)
p1 <- pdp::partial(rf.fit, pred.var = c("bmi"),
plot = TRUE, rug = TRUE,
plot.engine = "ggplot") + ggtitle("PDP (RF)")

png(file="PDP(boosting).png", height=1800, width=1800, units = "px", res=200)
p2 <- pdp::partial(gbm.fit, pred.var = c("bmi"),
plot = TRUE, rug = TRUE,
plot.engine = "ggplot") + ggtitle("PDP (boosting)")
...
```{r}
set.seed(1)
resamp = resamples(list(lm = lm.fit, lasso = lasso.fit, ridge = ridge.fit, enet = enet.fit, pls = pls.fit, gam = gam.fit, mars = mars.fit,rf = rf.fit,gbm = gbm.fit, rpart = rpart.fit))
summary(resamp)
